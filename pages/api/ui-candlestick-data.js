...
// /api/ui-candlestick-data

export default async function handler(req, res) {
    try {
        const { symbol, interval, limit } = req.query;
        if (!symbol || !interval) throw Error("symbol(query) & interval(query) is required")
        const URL = (limit) ? `https://www.binance.com/api/v3/uiKlines?symbol=${symbol.toUpperCase()}&interval=${interval}&limit=1000` : `https://www.binance.com/api/v3/klines?symbol=${symbol.toUpperCase()}&interval=${interval}`
